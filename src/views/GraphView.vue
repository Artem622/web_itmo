<template>
    ...
    <div :class="{ 'dark-theme': isDarkTheme, 'light-theme': isLightTheme }">
            <div class="container">
                <GraphComp ref="GraphComp" @callParentMethod="test"/>
...
export default {
    ...
    computed:{
        filteredItems() {
            const filteredArray = this.tableData.filter(item => item.r == this.$store.getters.getRadius);
    ...
    data(){
        return{
            debouncedTest: debounce(this.test, 200),
            debouncedDrawTest: debounce(this.drawTest, 100),
    ...
    methods:{
        ...
        drawTest(){
            try {
                let rad = this.$store.getters.getRadius
                let temp = this.tableData.filter(item => item.r == rad)
                if(temp.length > 0){
                    for(let j = 0; j < temp.length; j++){
                        this.$refs.GraphComp.drawPointFrom(temp[j].x, temp[j].y)
